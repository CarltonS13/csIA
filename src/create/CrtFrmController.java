 ...
 * @author SEGBEFIA_c
 */
public class CrtFrmController implements Initializable {
...
//    converts milli seconds back to time
   ...
   private void saveBtn(ActionEvent event){
       ...
       if (finalResult.isEmpty()) {
           ...
           mtnProgress.setProgress(0.25F);
           
           finalResult = finalResult + mtnCheck();
           
           mtnProgress.setProgress(0.50F);
   ...
   private void addToDatabase(){
    ...
    Time length = null;
        ...
        mtnQuery = "INSERT INTO `meetings`( `Name`, `Date`,"
               + " `Start Time`, `End Time`, `Length`, `Description`, "
               + "`Creator`) VALUES ('"+title+"',"
               + "'"+getUserDate()+"','"+start+"','"+end+"','"+length+"',"
               + "'"+desc+"','"+creator+"')" ;
       ...
       IDQuery = "SELECT * FROM `meetings` WHERE `Name` = '"+mtnTitle.getText()+"' AND `Date` = '"+getUserDate()+"' AND `Creator` ='"+creator+"'";
            mtnID = main.DataConn.returnID(IDQuery);
            saveMembers(mtnID);
       ...
       try {
            ...
            while (dbMembers.next()) {
                ...
                for (int i = 0; i< members.length;i++) {
                    ...
                    if (dbName.equalsIgnoreCase(members[i])) {
                        System.out.println("member clash");
                        isClash = true;
                        
                        mtnError.setText(dbName +
